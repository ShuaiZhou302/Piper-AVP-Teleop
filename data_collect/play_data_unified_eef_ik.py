...
def import_unified_converter():
    ...
    from unified_eef_pose import UnifiedEefConverter, matrix_to_rpy_xyz  # noqa: WPS433
    return UnifiedEefConverter, matrix_to_rpy_xyz


def unified_matrices_to_arm_rpy_poses(matrices, converter):
    """Convert T_unified_from_ee matrices to per-arm xyz+rpy IK targets."""
    _, matrix_to_rpy_xyz = import_unified_converter()
    poses = {}
...
def load_episode(path, urdf=DEFAULT_URDF):
    UnifiedEefConverter, _ = import_unified_converter()
    with h5py.File(path, "r") as f:
        ...
        for arm in ARM_ORDER:
            key = f"observations/ee_pose_in_unified/{arm}/matrix"
            if key not in f:
                raise KeyError(
    ...
    for arm in ARM_ORDER:
        if matrices[arm].shape != (T, 4, 4):
            raise ValueError(
                f"{arm} unified EE matrix shape should be ({T}, 4, 4), "
...
def main():
    ...
    episode = load_episode(args.episode, urdf=args.urdf)
